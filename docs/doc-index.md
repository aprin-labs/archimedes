...
| [`runbooks/dmarc-reports.md`](runbooks/dmarc-reports.md) | runbook | Dan Browne | 2026-09-04 | The DMARC aggregate-report inbox (#1504): the receipt-rule → private-S3 wiring that makes reports arrive at all, the scheduled weekly summary that mails the owner the table every Monday (and still mails one on a quiet week, so silence stays meaningful), how to read a per-source-IP pass/fail table, why `policy_evaluated` and not `auth_results` is DMARC, the four conditions that gate moving off `p=none`, the `quarantine`→`reject` ramp and how to roll it back, the diagnosis ladder for silence, and the separate one for objects that arrive and cannot be parsed — a third state that is neither a quiet week nor a clean one. Driven by `scripts/dmarc_report_summary.py` and `archimedes.scripts.dmarc_weekly_summary` — both exit non-zero rather than presenting an empty all-clear. |
