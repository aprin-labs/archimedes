...
- You only changed **backend Python** (e.g. PR #728 alone) — that ships zero contracts and is gated behind `supports_commit_reveal()`; merge and `docker compose restart`, no redeploy.
...
>
> **Note:** the task's assumed `scripts/deploy_contracts.py` does **not** exist. The Python deployer lives at `backend/archimedes/scripts/deploy_contracts.py`. Repo-root `scripts/` holds only `check-parity.sh`, `bulk_ingest_arxiv.py`, `fit_gmm_regime.py`, `setup-branch-protection.sh`. (A former EC2 `.env`-writer helper script was removed as a dead/dangerous file — it hardcoded stale addresses; edit `.env` directly per §5.1 instead.)
...
- The forge path does **not** produce it (use `run-latest.json` from §3.2). The `.mjs` path writes **directly into `.env`**; for the forge path, edit `.env` manually per §5.1 — not this file.
...
> There used to be an EC2 helper script that appended/replaced these **four core** ARC_ vars (`ARC_AMM_ROUTER_ADDRESS`, `ARC_VAULT_FACTORY_ADDRESS`, `ARC_REASONING_TRACE_REGISTRY_ADDRESS`, `ARC_ASSET_REGISTRY_ADDRESS`) in `.env` — it was removed as a dead/dangerous file because it hardcoded stale addresses. **Edit `.env` directly**, as shown above. It never covered `ARC_STRATEGY_REGISTRY_ADDRESS`/`ARC_SYNTHETIC_FACTORY_ADDRESS` or the per-asset synth/oracle vars either — those still need the manual edits in §5.2.
...
- **STEP 5 — #728 (commit/reveal v1.5), AUTOMATIC:** with the refreshed ABI, `supports_commit_reveal()` (`hasattr(registry.functions,'commit'/'reveal')`) now returns **True**, so `agent_runner` switches from the `publishTrace` fallback to the real commit→trade→reveal path — **with no code change**. Confirm: registry ABI now contains `commit`/`reveal`; agent logs show commit/reveal (not publishTrace fallback). Reveal is hash-only (empty `storagePointer`); IPFS pinning is not live ([ADR](../adr/ipfs-pinning-not-live.md)).
