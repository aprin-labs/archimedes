...
| `REQUEST_PATH_WARMUP` | `warmup_enabled()` — [`services/request_path_warmup.py`](../../backend/archimedes/services/request_path_warmup.py) | unset → code default `"true"` (ON). Not in `infra/ecs.tf`; not on the CI rewrite pin. Only the falsy literals (`0`/`false`/`no`/`off`) disable. Forced off under `TESTING`. | **LIVE on the warmup path ([#1713](https://github.com/aprin-labs/archimedes/issues/1713)).** Primes the Library-page caches (cohort daily returns, `strategies_list` rigor, selection-bias gate) in the FastAPI lifespan before `yield`, so a new Fargate task cannot become an ALB target with cold caches. Off is an emergency kill switch: `REQUEST_PATH_WARMUP=0` serves cold (the pre-#1713 behaviour) without a rollback. Not FLIP-AT-LAUNCH — production wants it ON. A timed-out warmup refuses to listen rather than joining cold. |
