...
export default function CorpusExplorer() {
	...
	return (
		<div className="corpus-explorer">
			<header className="app-page-heading corpus-header">
				<p className="app-eyebrow">Research index</p>
				<h1>Research corpus</h1>
				<p>
					Browse the literature behind a strategy. Inspect sources before
					drawing conclusions.
				</p>
            ...
            tab a visitor lands on first (#1356). */}
                ...
                pipeline has processed 0 of them (#778 item A / #1368). */}
							<span className="stat-chip">
								{overview.total_papers?.toLocaleString()} paper metadata records
							</span>
							<span className="stat-chip">
			...
			<div className="corpus-tabs" role="group" aria-label="Research views">
				{TABS.map((t) => (
					<button
						type="button"
						key={t}
						className={`corpus-tab${tab === t ? " active" : ""}`}
						aria-pressed={tab === t}
						onClick={() => setTab(t)}
					>
						{TAB_LABELS[t] ??
...
function OverviewTab({ overview, overviewError, onRetry }) {
	...
	if (!overview)
		return <div className="corpus-loading">Loading overview...</div>;

	const maxCatCount = Math.max(
		...(overview.categories || []).map((c) => c.count),
		1,
	);
	const maxYearCount = Math.max(
		...(overview.year_distribution || []).map((y) => y.count),
		1,
	);
			...
			<div className="overview-summary">
				...
				<div className="summary-grid">
					<div className="summary-card">
						...
						<div className="summary-label">Paper metadata records</div>
					</div>
					<div className="summary-card">
					...
					<div className="summary-card">
						<div className="summary-value">
							{overview.year_distribution?.length}
						</div>
		...
		<div className="corpus-catalog">
            ...
            and the field is then anonymous to a screen reader (3.3.2). */}
				<label className="catalog-search-field">
					...
					<input
						type="text"
						placeholder="Title, abstract or authors"
						value={search}
          ...
          semantic, ranked, or "smart". */}
			<p id="catalog-search-scope" className="catalog-search-scope">
				Substring match over title, abstract and author names —
				case-insensitive, no ranking. Author names are matched from 3 characters
				up.
			</p>

			{loading ? (
				<div className="corpus-loading">Loading...</div>
			) : catalogError ? (
						...
						<table className="catalog-table">
							...
							<tbody>
								{papers.map((p) => (
									...
									>
										<td title={cleanLatex(p.title) || p.arxiv_id}>
										...
										<td>
											{p.primary_category && (
												<span
													...
													title={
														p.category_label
															? `${p.primary_category} — ${p.category_label}`
															: p.primary_category
													}
												>
...
function PaperDetail({ paper, onBack }) {
	...
	const arxivAbsUrl = paper.arxiv_id
	...
	const safePdfUrl =
		typeof paper.pdf_url === "string" && /^https:\/\//i.test(paper.pdf_url)
			? paper.pdf_url
			: null;
	const arxivPdfUrl =
		safePdfUrl ||
	...
	return (
		<div className="corpus-explorer">
			<button className="back-btn flex items-center gap-1.5" onClick={onBack}>
				<span className="i-lucide-arrow-left w-4 h-4" /> Back to Explorer
			</button>
			<div className="paper-detail" style={{ maxWidth: 820 }}>
				<h1 className="paper-detail-title">
					{cleanLatex(paper.title) || paper.arxiv_id}
				</h1>
				...
				<div className="paper-detail-meta flex flex-wrap gap-2 mb-3.5">
						...
						<span
							...
							title={
								paper.category_label
									? `${paper.primary_category} — ${paper.category_label}`
									: paper.primary_category
							}
						>
					...
					{paper.topic_label && (
						<span className="tag tag-accent">Topic: {paper.topic_label}</span>
					)}
				</div>
				...
				>
					<h2 className="paper-detail-section-title">
						Cited by {paper.citing_strategies?.length || 0} strateg
						{paper.citing_strategies?.length === 1 ? "y" : "ies"}
					</h2>
					{paper.citing_strategies?.length > 0 ? (
									...
									<div>
										...
										{s.method && (
											<div className="caption" style={{ marginTop: 2 }}>
												...
												{s.status && (
													<span style={{ marginLeft: 6 }}>· {s.status}</span>
												)}
											</div>
										)}
									</div>
						...
						<p className="caption" style={{ color: "var(--text-4)" }}>
							No strategies in the library currently cite this paper. Generate
							one from{" "}
							<a href="/app/generate" style={{ color: "var(--accent)" }}>
								Generate
							</a>{" "}
							— when the fusion engine selects this paper, the link will appear
							here.
						</p>
					)}
				</div>
